...
// ─── Storage ──────────────────────────────────────────────────────────────────
const KEYS = { r:"fnp_r4", c:"fnp_c3", p:"fnp_p3", g:"fnp_gl" };
...
// ─── Logo — premium fork & pantry design ─────────────────────────────────────
function Logo({size=34}){
  return(
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="lgbg2" x1="0" y1="0" x2="48" y2="48" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#2E5435"/>
          <stop offset="60%" stopColor="#1E3A24"/>
          <stop offset="100%" stopColor="#142019"/>
        </linearGradient>
        <linearGradient id="lgsh2" x1="0" y1="0" x2="0" y2="28" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="white" stopOpacity=".22"/>
          <stop offset="100%" stopColor="white" stopOpacity="0"/>
        </linearGradient>
        <linearGradient id="lgjar" x1="14" y1="22" x2="34" y2="44" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#A8D5B0"/>
          <stop offset="100%" stopColor="#72A67A"/>
        </linearGradient>
      ...
      <rect width="48" height="48" rx="13" fill="url(#lgbg2)"/>
      {/* Subtle top sheen */}
      <rect width="48" height="28" rx="13" fill="url(#lgsh2)"/>
      {/* Subtle inner border */}
      <rect x=".75" y=".75" width="46.5" height="46.5" rx="12.5" stroke="white" strokeOpacity=".1" strokeWidth="1.5" fill="none"/>

      {/* Fork — left side */}
      {/* Tines */}
      <line x1="11" y1="8" x2="11" y2="18" stroke="#EDE8DC" strokeWidth="1.7" strokeLinecap="round"/>
      <line x1="14" y1="8" x2="14" y2="18" stroke="#EDE8DC" strokeWidth="1.7" strokeLinecap="round"/>
      <line x1="17" y1="8" x2="17" y2="18" stroke="#EDE8DC" strokeWidth="1.7" strokeLinecap="round"/>
      {/* Neck curve */}
      <path d="M11 18 Q11 23 14 24 Q17 23 17 18" stroke="#EDE8DC" strokeWidth="1.7" strokeLinecap="round" fill="none"/>
      {/* Handle */}
      <line x1="14" y1="24" x2="14" y2="40" stroke="#EDE8DC" strokeWidth="1.8" strokeLinecap="round"/>

      {/* Jar / pantry icon — right side */}
      {/* Lid */}
      <rect x="27" y="19" width="13" height="4" rx="2" fill="#B8D9BC" opacity=".95"/>
      {/* Lid band highlight */}
      <rect x="28" y="19.5" width="11" height="1.5" rx=".75" fill="white" opacity=".3"/>
      {/* Jar body */}
      <path d="M28.5 23 Q27 24 27 26 L27 38 Q27 40 33.5 40 Q40 40 40 38 L40 26 Q40 24 38.5 23 Z" fill="url(#lgjar)" opacity=".9"/>
      {/* Jar shine */}
      <ellipse cx="31" cy="29" rx="2" ry="4" fill="white" opacity=".15" transform="rotate(-15 31 29)"/>
      {/* Jar label line */}
      <line x1="29" y1="33" x2="38" y2="33" stroke="white" strokeOpacity=".3" strokeWidth="1" strokeLinecap="round"/>
      <line x1="30" y1="36" x2="37" y2="36" stroke="white" strokeOpacity=".2" strokeWidth="1" strokeLinecap="round"/>

      {/* Small leaf accent between fork and jar */}
      <path d="M21 20 Q22 17 24 18 Q22 21 21 20Z" fill="#72A67A" opacity=".85"/>
      <line x1="21" y1="20" x2="24" y2="18" stroke="#72A67A" strokeWidth=".8" opacity=".7"/>
    </svg>
  );
}
...
function Toast({msg}){
  if(!msg)return null;
  return(
    <div style={{position:"fixed",bottom:90,left:"50%",transform:"translateX(-50%)",background:"rgba(26,46,30,.92)",color:"#fff",borderRadius:24,padding:"10px 22px",fontSize:13,fontWeight:600,zIndex:9999,pointerEvents:"none",whiteSpace:"nowrap",boxShadow:"0 4px 20px rgba(0,0,0,.3)",backdropFilter:"blur(8px)"}}>
      {msg}
    </div>
  );
}
  ...
  return(
    <div onClick={()=>onOpen(recipe)} style={{background:"var(--cream)",borderRadius:"var(--r-lg)",border:"1px solid rgba(255,255,255,.85)",boxShadow:"var(--sh-sm)",overflow:"hidden",cursor:"pointer",transition:"transform .2s, box-shadow .2s",marginBottom:12}}
      ...
      <div style={{height:180,position:"relative"}}>
        ...
        {/* Delete button */}
        <button onClick={e=>{e.stopPropagation();onDelete(recipe.id);}} style={{position:"absolute",top:10,right:10,background:"rgba(15,24,17,.5)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,.2)",color:"#fff",borderRadius:"50%",width:28,height:28,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2}}>×</button>
        {/* Fav button */}
        {onToggleFav&&<button onClick={e=>{e.stopPropagation();onToggleFav(recipe.id);}} style={{position:"absolute",top:10,left:10,background:"rgba(15,24,17,.5)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,.2)",color:"#fff",borderRadius:"50%",width:28,height:28,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2}}>{recipe.fav?"❤️":"🤍"}</button>}
        {recipe.nutrition?.calories>0&&(
          <div style={{position:"absolute",top:10,left:recipe.fav!==undefined||onToggleFav?46:10,background:"rgba(15,24,17,.55)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,.2)",borderRadius:20,padding:"3px 10px",fontSize:11,color:"#fff",fontWeight:700}}>🔥 {recipe.nutrition.calories} cal</div>
        )}
      </div>
...
function CookMode({recipe,onClose}){
  ...
  function prev(){if(step>0){setStep(s=>s-1);setDone(false);}}
  function next(){
    if(step<total-1){setStep(s=>s+1);}
    else{setDone(true);}
  }

  const pct=total>0?Math.round(((done?total:step)/total)*100):0;

  return(
    <div style={{position:"fixed",inset:0,background:"#1A2E1E",zIndex:700,display:"flex",flexDirection:"column",paddingTop:"env(safe-area-inset-top)",paddingBottom:"env(safe-area-inset-bottom)"}}>
      {/* Progress bar */}
      <div style={{height:4,background:"rgba(255,255,255,.15)",flexShrink:0}}>
        <div style={{height:"100%",background:"linear-gradient(90deg,#72A67A,#A8D5B0)",width:`${pct}%`,transition:"width .4s ease",borderRadius:2}}/>
      </div>
      {/* Header */}
      <div style={{padding:"16px 20px 12px",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontFamily:"Lora,serif",fontSize:13,color:"rgba(255,255,255,.55)",fontWeight:500,marginBottom:2,textTransform:"uppercase",letterSpacing:"0.06em"}}>Cook Mode</div>
          <div style={{fontFamily:"Lora,serif",fontSize:17,color:"rgba(255,255,255,.9)",fontWeight:600,lineHeight:1.2}}>{recipe.title}</div>
        </div>
        <button onClick={onClose} style={{background:"rgba(255,255,255,.12)",border:"1px solid rgba(255,255,255,.2)",color:"rgba(255,255,255,.8)",borderRadius:"50%",width:36,height:36,fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
      </div>
      {/* Step area */}
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 28px"}}>
        {done?(
          <div style={{textAlign:"center"}}>
            ...
            <div style={{fontFamily:"Lora,serif",fontSize:28,fontWeight:600,color:"#fff",marginBottom:12}}>All done!</div>
            <div style={{fontSize:16,color:"rgba(255,255,255,.65)",lineHeight:1.7}}>Enjoy your {recipe.title}!</div>
          </div>
        ):(
          <>
            <div style={{background:"rgba(255,255,255,.08)",borderRadius:20,padding:"8px 20px",marginBottom:24,fontSize:13,color:"rgba(255,255,255,.6)",fontWeight:700,letterSpacing:"0.06em"}}>
              STEP {step+1} OF {total}
            </div>
            <p style={{fontFamily:"Lora,serif",fontSize:22,color:"#fff",lineHeight:1.75,textAlign:"center",fontWeight:500,maxWidth:340}}>
              {steps[step]}
            </p>
          </>
        )}
      ...
      {/* Controls */}
      <div style={{padding:"0 24px 24px",flexShrink:0,display:"flex",gap:14}}>
        {!done&&(
          <button onClick={prev} disabled={step===0} style={{flex:1,padding:"16px 0",borderRadius:16,border:"2px solid rgba(255,255,255,.25)",background:"transparent",color:step===0?"rgba(255,255,255,.3)":"rgba(255,255,255,.85)",fontSize:15,fontWeight:700,cursor:step===0?"default":"pointer",transition:"all .2s"}}>← Prev</button>
        )}
        <button onClick={done?onClose:next} style={{flex:done?undefined:1,padding:"16px 24px",borderRadius:16,border:"none",background:"linear-gradient(145deg,#4A7050,#2C4E34)",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",boxShadow:"0 4px 16px rgba(58,94,66,.4)",transition:"all .2s",minWidth:done?160:undefined}}>
          {done?"Close ✓":`Next →`}
        </button>
      </div>
    </div>
...
// ─── Recipe detail modal ──────────────────────────────────────────────────────
function RecipeModal({recipe,onClose,onUpdate,onStartCook,onToggleFav}){
  ...
  useEffect(()=>{ if(recipe){setServings(recipe.servings||4);setEditing(false);setCheckedIngs(new Set());} },[recipe]);
  ...
  function toggleIng(i){
    setCheckedIngs(s=>{const n=new Set(s);n.has(i)?n.delete(i):n.add(i);return n;});
  }

  function startTimer(label,timeStr){
    if(timerRef.current)clearInterval(timerRef.current);
    const secs=parseTimeSecs(timeStr);
    if(!secs)return;
    let remaining=secs;
    const id=setInterval(()=>{
      ...
      if(remaining<=0){
        ...
        setTimer(null);
        // Show alert
        if(typeof window!=="undefined")alert(`⏰ Time's up! ${label} done`);
      } else {
        setTimer(t=>t?{...t,remaining}:null);
      }
    },1000);
    timerRef.current=id;
    setTimer({label,totalSecs:secs,remaining,running:true});
  }
  ...
  return(
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,24,17,.65)",backdropFilter:"blur(5px)",WebkitBackdropFilter:"blur(5px)",zIndex:500,display:"flex",alignItems:"flex-end"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"var(--linen)",borderRadius:"24px 24px 0 0",width:"100%",maxHeight:"95vh",overflowY:"auto",paddingBottom:"calc(24px + env(safe-area-inset-bottom))",boxShadow:"0 -8px 48px rgba(15,24,17,.25)"}}>
        {/* Active timer pill */}
        {timer&&(
          <div style={{position:"sticky",top:0,zIndex:10,background:"linear-gradient(90deg,#1A2E1E,#2C4E34)",padding:"10px 18px",display:"flex",alignItems:"center",gap:12}}>
            <span style={{fontSize:16}}>⏱</span>
            <div style={{flex:1}}>
              <span style={{color:"#fff",fontSize:13,fontWeight:600}}>{timer.label}</span>
              <span style={{color:"rgba(255,255,255,.7)",fontSize:13,marginLeft:10}}>{fmtMMSS(timer.remaining)}</span>
            </div>
            <button onClick={cancelTimer} style={{background:"none",border:"none",color:"rgba(255,255,255,.6)",fontSize:18,cursor:"pointer",lineHeight:1,padding:"0 4px"}}>✕</button>
          </div>
        )}
        {/* Hero */}
        <div style={{height:260,position:"relative",flexShrink:0,borderRadius:"24px 24px 0 0",overflow:"hidden"}}>
          ...
          <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"20px 20px 18px"}}>
            ...
            <div style={{display:"flex",gap:12,fontSize:12,color:"rgba(255,255,255,.75)"}}>
              ...
              {recipe.prepTime&&(
                <button onClick={()=>startTimer("Prep",recipe.prepTime)} style={{background:"none",border:"none",padding:0,cursor:"pointer",color:"rgba(255,255,255,.75)",fontSize:12,fontFamily:"inherit"}}>⏱ {recipe.prepTime}</button>
              )}
              {recipe.cookTime&&(
                <button onClick={()=>startTimer("Cook",recipe.cookTime)} style={{background:"none",border:"none",padding:0,cursor:"pointer",color:"rgba(255,255,255,.75)",fontSize:12,fontFamily:"inherit"}}>🔥 {recipe.cookTime}</button>
              )}
            </div>
          </div>
          {/* Close button */}
          ...
          {/* Edit button */}
          <button onClick={()=>setEditing(true)} style={{position:"absolute",top:14,right:56,background:"rgba(15,24,17,.5)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,.25)",color:"#fff",borderRadius:20,padding:"7px 13px",fontSize:12,fontWeight:700,cursor:"pointer"}}>✏️ Edit</button>
          {/* Fav button */}
          {onToggleFav&&<button onClick={()=>onToggleFav(recipe.id)} style={{position:"absolute",top:56,right:56,background:"rgba(15,24,17,.5)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,.25)",color:"#fff",borderRadius:20,padding:"7px 13px",fontSize:12,fontWeight:700,cursor:"pointer"}}>{recipe.fav?"❤️":"🤍"}</button>}
        </div>
        ...
        <div style={{padding:"18px 20px 0"}}>
          ...
          {recipe.ingredients?.length>0&&<>
            ...
            <ul style={{listStyle:"none",marginBottom:20}}>
              {recipe.ingredients.map((ing,i)=>{
                ...
                return(
                  <li key={i} onClick={()=>toggleIng(i)} style={{fontSize:14,color:isChecked?"var(--mist)":"var(--ink)",padding:"8px 0",borderBottom:"1px solid var(--sage-pale)",display:"flex",gap:10,alignItems:"baseline",cursor:"pointer",textDecoration:isChecked?"line-through":"none",opacity:isChecked?0.45:1,transition:"all .2s"}}>
                    <span style={{color:isChecked?"var(--moss)":"var(--sage)",fontSize:isChecked?11:9,flexShrink:0}}>{isChecked?"✓":"◆"}</span>{line}
                  </li>
                );
              })}
            </ul>
          ...
          {/* Cook Mode button */}
          {recipe.steps?.length>0&&onStartCook&&(
            <button onClick={()=>onStartCook(recipe)} className="btn-primary" style={{width:"100%",padding:"13px 0",fontSize:14,borderRadius:"var(--r-md)",marginBottom:16,letterSpacing:"0.02em"}}>▶ Start Cooking</button>
          )}
...
function AddSheet({onAdd,onClose,prefill="",existingRecipes=[]}){
  ...
  async function doApiCall({text="",imageBase64="",imageMediaType="image/jpeg"}){
    setLoading(true);setError("");setDupWarning(false);setPendingPayload(null);
    try{
  ...
  async function parseAndSave({text="",imageBase64="",imageMediaType="image/jpeg"}){
    ...
    if(text.trim().startsWith("http")){
      const url=text.trim();
      if(existingRecipes.some(r=>r.url===url)){
        setPendingPayload({text,imageBase64,imageMediaType});
        setDupWarning(true);
        return;
      }
    }
    await doApiCall({text,imageBase64,imageMediaType});
  }
  ...
  if(dupWarning&&pendingPayload) return(
    <Sheet onClose={onClose} tall>
      <div style={{padding:"24px 20px"}}>
        <div style={{fontSize:44,textAlign:"center",marginBottom:16}}>⚠️</div>
        <div className="serif" style={{fontWeight:600,fontSize:20,color:"var(--forest)",textAlign:"center",marginBottom:10}}>Already saved!</div>
        <div style={{fontSize:14,color:"var(--mist)",textAlign:"center",lineHeight:1.75,marginBottom:24}}>You already saved this recipe URL. Save again anyway?</div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>{setDupWarning(false);setPendingPayload(null);}} className="btn-ghost" style={{flex:1,padding:"13px 0",fontSize:14,borderRadius:"var(--r-md)"}}>Cancel</button>
          <button onClick={()=>doApiCall(pendingPayload)} className="btn-primary" style={{flex:1,padding:"13px 0",fontSize:14,borderRadius:"var(--r-md)"}}>Save anyway</button>
        </div>
      </div>
    </Sheet>
  );
    ...
    <Sheet onClose={onClose} tall>
      <div style={{padding:"14px 18px 24px"}}>
        ...
        {tab==="paste"&&<>
          ...
          <button onClick={()=>parseAndSave({text:input})} disabled={!input.trim()} className="btn-primary" style={{width:"100%",padding:"14px 0",marginTop:10,borderRadius:"var(--r-md)",opacity:input.trim()?1:.6}}>Save with AI ✦</button>
...
// ─── RECIPES TAB ──────────────────────────────────────────────────────────────
function RecipesTab({recipes,onAdd,onDelete,onUpdate,sharedPrefill,clearShared,onToggleFav}){
  ...
  let filtered=recipes.filter(r=>{
    ...
    const matchSearch=(!q||r.title?.toLowerCase().includes(q)||(r.tags||[]).some(t=>t.toLowerCase().includes(q))||r.source?.toLowerCase().includes(q));
    const matchTag=(!tag||(r.tags||[]).includes(tag));
    const matchFav=(!showFavs||r.fav===true);
  ...
  return(
    <div style={{flex:1,overflowY:"auto",paddingBottom:90}}>
      ...
      {/* Tag pills row with Favs toggle */}
      <div style={{display:"flex",gap:6,padding:"10px 16px 0",overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
        {/* Favourites toggle */}
        <button onClick={()=>setShowFavs(f=>!f)} style={{flexShrink:0,background:showFavs?"#9D174D":"var(--cream)",color:showFavs?"#fff":"var(--ink)",border:"1px solid var(--sage-lt)",borderRadius:20,padding:"4px 13px",fontSize:11,fontWeight:700,cursor:"pointer",textTransform:"uppercase",letterSpacing:"0.04em",transition:"all .15s",boxShadow:showFavs?"var(--sh-xs)":"none"}}>❤️ Favs</button>
        {allTags.map(t=>(
        ...
        {tag&&<button onClick={()=>setTag("")} style={{flexShrink:0,background:"#FEE2E2",color:"#991B1B",border:"1px solid #FECACA",borderRadius:20,padding:"4px 11px",fontSize:11,fontWeight:700,cursor:"pointer"}}>✕ Clear</button>}
      </div>
      ...
      <div style={{padding:"10px 16px 0"}}>
        {filtered.length===0?(
          <div style={{textAlign:"center",paddingTop:80}}>
            ...
            <div className="serif" style={{fontWeight:600,fontSize:22,color:"var(--forest)",marginBottom:8}}>{recipes.length===0?"Your pantry is empty":showFavs?"No favourites yet":"Nothing matches"}</div>
            <div style={{fontSize:14,color:"var(--mist)",lineHeight:1.8}}>{recipes.length===0?"Tap + Add to save your first recipe.":showFavs?"Tap ❤️ on a recipe to favourite it.":"Try a different search or tag."}</div>
          </div>
        ):filtered.map(r=><RecipeCard key={r.id} recipe={r} onOpen={setSelected} onDelete={onDelete} onToggleFav={onToggleFav}/>)}
      </div>
      ...
      {showAdd&&<AddSheet onAdd={r=>{onAdd(r);setShowAdd(false);clearShared();}} onClose={()=>{setShowAdd(false);clearShared();}} prefill={sharedPrefill} existingRecipes={recipes}/>}
      <RecipeModal recipe={selected} onClose={()=>setSelected(null)} onUpdate={r=>{onUpdate(r);setSelected(r);}} onStartCook={r=>{setSelected(null);setCookingRecipe(r);}} onToggleFav={onToggleFav}/>
...
// ─── Grocery aggregation helpers ──────────────────────────────────────────────
function parseIngLine(line){
  // Try to match: amount unit name
  const m=line.match(/^([\d./\s⅛¼⅓½⅔¾]+)\s*(g|kg|ml|L|tsp|tbsp|cup|oz|lb|fl\s*oz)\s+(.+)$/i);
  if(m){
    const amount=parseFloat(m[1].trim())||1;
    const unit=m[2].trim().toLowerCase().replace(/\s+/g,"");
    const name=m[3].trim().toLowerCase();
    return{amount,unit,name};
  }
  // No unit — try amount + name
  const m2=line.match(/^([\d./\s⅛¼⅓½⅔¾]+)\s+(.+)$/);
  if(m2){
    const amount=parseFloat(m2[1].trim());
    if(amount&&!isNaN(amount))return{amount,unit:null,name:m2[2].trim().toLowerCase()};
  }
  return{amount:null,unit:null,name:line.trim().toLowerCase()};
}

const UNIT_NORM_G={g:1,kg:1000,oz:28.35,lb:453.59};
const UNIT_NORM_ML={ml:1,l:1000,tsp:4.93,tbsp:14.79,cup:236.59,"floz":29.57};

function normaliseToBase(amount,unit){
  if(!unit||!amount)return{base:amount,type:"count"};
  const u=unit.toLowerCase().replace(/\s/g,"");
  if(UNIT_NORM_G[u])return{base:amount*UNIT_NORM_G[u],type:"weight"};
  if(UNIT_NORM_ML[u])return{base:amount*UNIT_NORM_ML[u],type:"volume"};
  return{base:amount,type:"count"};
}

function bestWeightUnit(grams){
  if(grams>=900)return{val:grams/1000,unit:"kg"};
  if(grams>=1)return{val:grams,unit:"g"};
  return{val:grams*1000,unit:"mg"};
}
function bestVolumeUnit(ml){
  if(ml>=200)return{val:ml/236.59,unit:"cup"};
  if(ml>=14)return{val:ml/14.79,unit:"tbsp"};
  if(ml>=4)return{val:ml/4.93,unit:"tsp"};
  return{val:ml,unit:"ml"};
}

function aggregateIngredients(rawItems){
  // rawItems: [{id, text, recipe, checked}]
  // Parse each, aggregate by name+type
  const map=new Map();
  for(const item of rawItems){
    const parsed=parseIngLine(item.text);
    const key=parsed.name;
    if(!map.has(key)){
      map.set(key,{...parsed,recipes:[item.recipe],origText:item.text,id:item.id});
    } else {
      const ex=map.get(key);
      const {base:b1,type:t1}=normaliseToBase(ex.amount,ex.unit);
      const {base:b2,type:t2}=normaliseToBase(parsed.amount,parsed.unit);
      if(t1===t2&&t1!=="count"&&b1!==null&&b2!==null){
        const newBase=b1+b2;
        let newUnit=ex.unit,newAmount;
        if(t1==="weight"){const r=bestWeightUnit(newBase);newUnit=r.unit;newAmount=r.val;}
        else{const r=bestVolumeUnit(newBase);newUnit=r.unit;newAmount=r.val;}
        map.set(key,{...ex,amount:newAmount,unit:newUnit,recipes:[...ex.recipes,item.recipe]});
      } else if(t1==="count"&&t2==="count"&&ex.amount!==null&&parsed.amount!==null){
        map.set(key,{...ex,amount:(ex.amount||0)+(parsed.amount||0),recipes:[...ex.recipes,item.recipe]});
      } else {
        // Can't aggregate — keep first, append recipe ref
        map.set(key,{...ex,recipes:[...ex.recipes,item.recipe]});
      }
    }
  }
  ...
  for(const[name,data] of map){
    ...
    if(data.amount!==null&&data.unit){
      text=`${fmtN(data.amount)} ${data.unit} ${name}`;
    } else if(data.amount!==null&&!data.unit){
      text=`${fmtN(data.amount)} ${name}`;
    } else {
      text=data.origText||name;
    }
    const recipe=[...new Set(data.recipes)].join(", ");
    result.push({id:`agg_${idx++}`,text,recipe,checked:false});
  }
  return result;
}
    ...
    for(const r of unique){
      if(r.ingredients?.length>0){
        for(const ing of r.ingredients){
          ...
          rawItems.push({id:Date.now().toString()+Math.random(),text:line,recipe:r.title,checked:false});
        }
      }
    }
    return aggregateIngredients(rawItems);
  }
  ...
  return(
    <div style={{flex:1,overflowY:"auto",paddingBottom:90}}>
      <div style={{padding:"12px 16px 10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          {plannedCount>0&&<button onClick={()=>setShowGrocery(true)} className="btn-primary" style={{padding:"7px 13px",fontSize:12,borderRadius:20}}>🛒 Grocery list</button>}
      ...
      {showGrocery&&<GrocerySheet initItems={buildGroceryList()} onClose={()=>setShowGrocery(false)}/>}
      <RecipeModal recipe={recipeModal} onClose={()=>setRecipeModal(null)} onUpdate={r=>{if(onUpdate)onUpdate(r);setRecipeModal(r);}}/>
    </div>
  );
}
...
// ─── GROCERY LIST ─────────────────────────────────────────────────────────────
function GrocerySheet({initItems,onClose}){
  ...
  // Persist on change
  useEffect(()=>{ save(KEYS.g,items); },[items]);

  // When initItems changes (new grocery list from planner), merge in
  useEffect(()=>{
    if(initItems&&initItems.length>0){
      setItems(initItems);
    }
  },[]);// Only on mount — initItems is the new list from planner
...
// ─── Header with menu ─────────────────────────────────────────────────────────
function Header({count,onExport,onImport}){
  ...
  function handleImportFile(e){
    ...
    reader.onload=ev=>{
      try{
        ...
        if(Array.isArray(data)&&onImport){onImport(data);}
  ...
  return(
    <div style={{background:"linear-gradient(180deg,#243529 0%,#1A2E1E 100%)",paddingTop:"env(safe-area-inset-top)",flexShrink:0,boxShadow:"0 3px 16px rgba(15,24,17,.22)",position:"relative",zIndex:200}}>
      <div style={{padding:"12px 18px 13px",display:"flex",alignItems:"center",gap:11}}>
        ...
        {/* Menu button */}
        <button onClick={()=>setMenuOpen(o=>!o)} style={{background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.15)",color:"rgba(255,255,255,.85)",borderRadius:"50%",width:36,height:36,fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>⋮</button>
      </div>
      {/* Dropdown menu */}
      {menuOpen&&(
        <>
          <div style={{position:"fixed",inset:0,zIndex:199}} onClick={()=>setMenuOpen(false)}/>
          <div style={{position:"absolute",top:"100%",right:14,background:"var(--cream)",borderRadius:"var(--r-md)",boxShadow:"var(--sh-lg)",border:"1px solid var(--parchment)",minWidth:200,zIndex:200,overflow:"hidden"}}>
            <div style={{padding:"10px 16px 6px",borderBottom:"1px solid var(--parchment)"}}>
              <div style={{fontSize:12,color:"var(--mist)",fontWeight:600}}>{count} recipe{count!==1?"s":""} in your pantry</div>
            </div>
            <button onClick={()=>{onExport&&onExport();setMenuOpen(false);}} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"12px 16px",border:"none",background:"none",cursor:"pointer",fontSize:14,color:"var(--forest)",textAlign:"left",fontWeight:500}}>
              <span style={{fontSize:18}}>⬇️</span> Export recipes
            </button>
            <button onClick={()=>{ fileRef.current?.click(); }} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"12px 16px",border:"none",background:"none",cursor:"pointer",fontSize:14,color:"var(--forest)",textAlign:"left",fontWeight:500,borderTop:"1px solid var(--sage-pale)"}}>
              <span style={{fontSize:18}}>⬆️</span> Import recipes
            </button>
          </div>
        </>
      )}
...
function AppInner(){
  ...
  useEffect(()=>{
    function handlePopState(){
      // Check if any modal state is open by checking DOM
      // Since modals are rendered by child components, we use a simple approach:
      // push a state when modals open; popstate fires, we just show the exit toast
      // (Modal closing on back is handled by each modal's own popstate listener)
      const now=Date.now();
      if(now-lastBackTimeRef.current<1500){
        // Second back — let it go (do nothing, browser will navigate away)
        return;
      }
      lastBackTimeRef.current=now;
      // Push state again so browser doesn't actually navigate away
  ...
  function toggleFav(id){
    const u=recipes.map(r=>r.id===id?{...r,fav:!r.fav}:r);
    setRecipes(u);save(KEYS.r,u);
  }
  ...
  function exportRecipes(){
    ...
    a.href=url;
    a.download="fork-n-pantry-backup.json";
    a.click();
    URL.revokeObjectURL(url);
  }

  function importRecipes(data){
    ...
    setRecipes(merged);
    save(KEYS.r,merged);
    setToast(`Imported ${data.length} recipe${data.length!==1?"s":""}`);
    setTimeout(()=>setToast(""),2000);
  }

  return(
    <div style={{height:"100dvh",display:"flex",flexDirection:"column",background:"var(--linen)"}}>
      <Header count={recipes.length} onExport={exportRecipes} onImport={importRecipes}/>
      {tab==="recipes"&&<RecipesTab recipes={recipes} onAdd={addRecipe} onDelete={deleteRecipe} onUpdate={updateRecipe} sharedPrefill={sharedPrefill} clearShared={()=>setSharedPrefill("")} onToggleFav={toggleFav}/>}
